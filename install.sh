...
MODULE_NAME="heroku"
REPO_URL="${RATKO_REPO_URL:-${HEROKU_REPO_URL:-https://github.com/unsidogandon/ratko.git}}"
REPO_REF="${RATKO_REPO_REF:-main}"
VENV_DIR="${RATKO_VENV_DIR:-${HEROKU_VENV_DIR:-.venv}}"
LOG_FILE="ratko-install.log"
...
prepare_repo() {
	...
	info "Cloning repo..."
	if [ -e "$APP_DIR" ]; then
		fail "Target path already exists: $APP_DIR" 3
	fi
	"${RUN_AS_USER[@]}" git clone --branch "$REPO_REF" --single-branch "$REPO_URL" "$APP_DIR" >>"$LOG_FILE" 2>&1 || fail "Clone failed." 3
	cd "$APP_DIR"
}
...
install_python_packages() {
	...
	"${RUN_AS_USER[@]}" "$venv_python" -c 'import hashlib, pathlib; p = pathlib.Path("requirements.txt"); pathlib.Path(".requirements_hash").write_text(hashlib.sha256(p.read_bytes()).hexdigest())' >>"$LOG_FILE" 2>&1 || fail "Requirements hash creation failed." 4
}
