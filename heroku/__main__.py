...
def get_data_root():
    ...
    for index, arg in enumerate(sys.argv):
        if arg == "--data-root" and index + 1 < len(sys.argv):
            cli_root = Path(sys.argv[index + 1]).expanduser()

        if arg.startswith("--data-root="):
            cli_root = Path(arg.split("=", maxsplit=1)[1]).expanduser()

    if cli_root is not None:
        return cli_root

    if configured_root := (
        os.environ.get("RATKO_DATA_ROOT") or os.environ.get("HEROKU_DATA_ROOT")
    ):
...
os.environ["HEROKU_DATA_ROOT"] = str(DATA_ROOT)
os.environ["RATKO_DATA_ROOT"] = str(DATA_ROOT)
...
if (
    ...
    print("\U0001f6ab" * 15)
    print("You attempted to run Ratko on behalf of root user")
