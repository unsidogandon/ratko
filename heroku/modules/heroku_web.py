...
class HerokuWebMod(loader.Module):
    ...
    async def _save_switch_session(
        ...
        session = SQLiteSession(os.path.join(main.SESSIONS_DIR, f"ratko-{new_id}"))
        session.set_dc(
    ...
    async def _switch_account(self, client: CustomTelegramClient) -> None:
        ...
        new_session = base / f"ratko-{new_id}.session"
        new_config = Path(main.BASE_PATH) / f"config-{new_id}.json"
        old_session_bases = {
            Path(self._client.session.filename),
            base / f"ratko-{old_id}.session",
            base / f"heroku-{old_id}.session",
        }
        new_session_bases = {
            new_session,
            base / f"heroku-{new_id}.session",
        }
        old_session_files = {
            Path(f"{session}{sidecar}")
            for session in old_session_bases
            for sidecar in ("", "-journal", "-wal", "-shm")
        }
        new_session_files = {
            Path(f"{session}{sidecar}")
            for session in new_session_bases
            for sidecar in ("", "-journal", "-wal", "-shm")
        }
        new_files = {*new_session_files, new_config}
        redis = getattr(self._db, "_redis", None)
        redis_backup = None

        db_data = self._clone_switch_db(old_id, new_id)
        db_data.setdefault("heroku.inline", {})[
            "custom_bot"
        ] = main.generate_inline_bot_username()
        archived = []
        moved_modules = []
        ...
        try:
            ...
            for path in (*old_session_files, old_config, *new_session_files, new_config):
