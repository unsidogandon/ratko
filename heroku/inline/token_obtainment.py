...
class TokenObtainment(InlineUnit):
    async def _create_bot(self: "InlineManager"):
        ...
        async with self._client.conversation("@BotFather", exclusive=False) as conv:
            ...
            custom_bot = self._db.get("heroku.inline", "custom_bot", False)
            if not custom_bot:
                logger.error("Custom inline bot username is not configured")
                return False

            username = f"@{custom_bot.strip('@')}"
            try:
                await self._client.get_entity(username)
            except ValueError:
                pass
            else:
                logger.error("Custom inline bot username %s is already occupied", username)
                return False
            ...
            try:
                ...
                if "DOCKER" in os.environ:
                    ...
                    local_banner = main.BASE_PATH / "banner.jpg"
                    m = await conv.send_file(
                        local_banner
                        if local_banner.exists()
                        else "https://raw.githubusercontent.com/unsidogandon/ratko/main/banner.jpg"
                    )
                r = await conv.get_response()
