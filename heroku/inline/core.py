...
class InlineManager(
    ...
    async def register_manager(
        ...
        for folder in _folders.filters:
            ...
            if getattr(folder_title, "text", folder_title) in {"Heroku", "Ratko"}:
                if any(
                    [
                ...
                bot_peer = await self._client.get_input_entity(self.bot_id)
                folder.pinned_peers = [
                    bot_peer,
                    *[
                        peer
                ...
                if hasattr(folder, "include_peers") and bot_peer not in folder.include_peers:
                    folder.include_peers = [bot_peer, *folder.include_peers]
    ...
    def unregister_bot_update_handler(self, handler_id: str):
        ...
        handler_ref = self._bot_handler_refs.pop(handler_id, None)
        logger.debug("Unregistered bot update handler %s", handler_id)

        if not self._bot_client or not handler_ref:
            return
