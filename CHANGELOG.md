...
- rebased the maintained fork on the current Heroku beta architecture
- migrated inline handling from aiogram to the current herokutl bot client
- removed the external beta allowlist and destructive startup reset
- added migration for legacy Ratko and Heroku session filenames
- made self-updates fast-forward-only and compatible with virtual environments
- rebuilt Docker images from the checked-out Ratko source with persistent `/data`
- fixed inline shutdown, custom handler unload and synchronous module unload hooks
- restored `hshakal` with bounded temporary storage and FFmpeg execution
- updated dependencies, installation scripts, repository links and documentation
